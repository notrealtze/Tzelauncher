...
object ResourceImporter {
    ...
    fun importFile(context: Context, uri: Uri): String {
        return try {
            val fileName = DocumentFile.fromSingleUri(context, uri)?.name ?: return "Could not read file name"
    ...
    fun importDirectory(context: Context, treeUri: Uri): String {
        return try {
            val documentFile = DocumentFile.fromTreeUri(context, treeUri) ?: return "Cannot access folder"
    ...
    private fun copyDocumentDirectory(context: Context, sourceDir: DocumentFile, destDir: File) {
        destDir.mkdirs()
